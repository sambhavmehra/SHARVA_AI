...
Provide a detailed report with specific line references and remediation recommendations."""
    ...
    def perform_recon(self, target):
      """Perform passive recon on a target (IP or domain) and return an AI-analyzed report."""
      if not self._validate_target(target):
        return f"[!] Invalid target: '{target}'. Please provide a valid IP address or domain name."

      recon_type = "IP" if self._is_ip_address(target) else "domain"
      ...
      try:
        if recon_type == "IP":
            report = self._perform_ip_recon(target, report)
        else:
            report = self._perform_domain_recon(target, report, recon_type="passive")

        return self._analyze_recon_data(report)

      except Exception as e:
        return f"[!] Error performing recon on {target}: {str(e)}"
...
"""
    ...
    def _perform_domain_recon(self, domain, report, recon_type):
      """Perform domain reconnaissance with WHOIS, DNS, SSL, and external lookups."""
    ...
    # Get DNS information
      try:
        ...
        report["results"]["domain_info"]["dns"] = {"error": f"DNS resolution failed: {e}"}
    ...
    # Get WHOIS information
      try:
        ...
        report["results"]["domain_info"]["whois"] = {"error": f"WHOIS lookup failed: {e}"}
    ...
    # Get SSL certificate information
      try:
        ...
        report["results"]["domain_info"]["ssl"] = {"error": f"SSL certificate lookup failed: {e}"}

    # Active scanning (if recon_type is "full")
      if recon_type == "full":
        try:
            report["results"]["port_scan"] = self._perform_port_scan(report["results"]["domain_info"]["dns"]["ip_addresses"][0])
        except Exception as e:
            report["results"]["port_scan"] = {"error": f"Port scan failed: {e}"}
    ...
    # Shodan lookup
      if self.shodan_api_key:
        try:
            ...
            report["results"]["shodan"] = {"error": f"Shodan lookup failed: {e}"}
    ...
    # VirusTotal lookup
      if self.virustotal_api_key:
        try:
            ...
            report["results"]["virustotal"] = {"error": f"VirusTotal lookup failed: {e}"}
